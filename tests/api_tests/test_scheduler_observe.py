...
def test_scheduler_observe_fail_when_enabled_and_logs_stale(monkeypatch):
    now = datetime.now(timezone.utc)
    report = {
        "latest results_sync ops_log": {"started_at": now - timedelta(minutes=61), "status": "ok"},
        "latest settlement_runner ops_log": {"started_at": now - timedelta(minutes=46), "status": "ok"},
        "latest odds_snapshots fetched_at": now,
        "open_bets_count": 0,
    }

    assert scheduler_observe._result(report, True, now) == "FAIL"


def test_scheduler_observe_wait_when_enabled_and_logs_missing():
    report = {
        "latest results_sync ops_log": None,
        "latest settlement_runner ops_log": None,
        "latest odds_snapshots fetched_at": None,
    ...
    assert scheduler_observe._result(report, True, datetime.now(timezone.utc)) == "WAIT"


def test_scheduler_observe_fail_when_job_error():
    now = datetime.now(timezone.utc)
    report = {
        "latest results_sync ops_log": {"started_at": now, "status": "error"},
        "latest settlement_runner ops_log": {"started_at": now, "status": "ok"},
        "latest odds_snapshots fetched_at": now,
        "open_bets_count": 0,
    }
