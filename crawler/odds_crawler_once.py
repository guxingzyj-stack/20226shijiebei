...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run one safe P0 odds crawler pass.")
...
def run_once(*, full_scan: bool, days_ahead: int | None) -> dict[str, object]:
    ...
    with db.connect() as conn:
        ...
        try:
            ...
            odds_rows_written = db.write_odds_snapshots(conn, matches, m500.SOURCE_NAME)
            max_kickoff = db.max_match_kickoff(conn)
            db.finish_run(conn, run_id, m500.SOURCE_NAME, len(matches), odds_rows_written, True, None)
        except Exception as exc:
            db.finish_run(conn, run_id, m500.SOURCE_NAME, len(matches), 0, False, str(exc))
            raise

    scan_summary = m500.get_last_scan_summary()
    return {
        "source": "500",
        "full_scan": full_scan,
        "raw_events_seen": scan_summary.get("raw_events_seen"),
        "parsed_matches_seen": scan_summary.get("parsed_matches_seen"),
        ...
        "odds_rows_written": odds_rows_written,
        "latest_kickoff_seen": scan_summary.get("latest_kickoff_seen"),
